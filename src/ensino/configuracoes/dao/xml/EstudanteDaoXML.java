...
public class EstudanteDaoXML extends AbstractDaoXML<Estudante> {
    ...
    public EstudanteDaoXML() throws IOException, ParserConfigurationException, TransformerException {
    ...
    public Estudante createObject(Element e, Object ref) {
        try {
            ...
            if (ref != null && ref instanceof Turma) {
                ...
                DaoPattern<Turma> dao = new TurmaDaoXML();
    ...
    public void save(Estudante o) {
        ...
        Integer campusId = curso.getCampus().getId(),
                ...
                turmaId = turma.getId();
        if (o.getId() == null) {
            o.setId(this.nextVal(turmaId, cursoId, campusId));
        }
        // cria a expressão de acordo com o código do campus
        String filter = String.format("@id=%d and @turmaId=%d and @cursoId=%d and @campusId=%d",
                o.getId(), turmaId, cursoId, campusId);
        super.save(o, filter);
    }
