...
public class PeriodoLetivoDaoXML extends AbstractDaoXML<PeriodoLetivo> {
    ...
    public PeriodoLetivoDaoXML(URL url) throws IOException, ParserConfigurationException, TransformerException {
        super("periodoLetivo", url, "PeriodoLetivo", "periodoLetivo", PeriodoLetivoFactory.getInstance());
    }
...
//            o.setSemanasLetivas(semanaLetivaDao.list(filter, o));
    ...
    public void save(PeriodoLetivo o) {
        Integer ano = o.getId().getCalendario().getId().getAno();
        Integer campusId = o.getId().getCalendario().getId().getCampus().getId();
        if (o.getId().getNumero() == null) {
            o.getId().setNumero(this.nextVal(ano, campusId));
        } 
        if (o.isDeleted()) {
            ...
            String filter = String.format("@numero=%d and @ano=%d and @campusId=%d",
                    o.getId().getNumero(), ano, campusId);
            super.save(o, filter);
        }
    }
