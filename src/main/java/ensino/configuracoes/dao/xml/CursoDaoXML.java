...
public class CursoDaoXML extends AbstractDaoXML<Curso> {
    ...
    public CursoDaoXML(URL url) throws IOException, ParserConfigurationException, TransformerException {
        super("curso", url, "Curso", "curso", CursoFactory.getInstance());
    }
...
//            o.setUnidadesCurriculares(unidadeCurricularDao.list(filter, o));
    ...
    public void save(Curso o) {
        Campus c = o.getId().getCampus();
        if (o.getId() == null) {
            o.getId().setId(this.nextVal(c.getId()));
        }
    ...
    public void delete(Curso o) {
        // Cria mecanismo para buscar o conteudo no xml
        String filter = String.format("@id=%d and @campusId=%d",
                o.getId().getId(), o.getId().getCampus().getId());
        super.delete(filter);
    }
