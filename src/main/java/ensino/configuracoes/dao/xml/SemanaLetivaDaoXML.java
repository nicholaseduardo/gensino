...
public class SemanaLetivaDaoXML extends AbstractDaoXML<SemanaLetiva> {
    ...
    public SemanaLetivaDaoXML(URL url) throws IOException, ParserConfigurationException, TransformerException {
        super("semanaLetiva", url, "SemanaLetiva", "semanaLetiva", SemanaLetivaFactory.getInstance());
    }
    ...
    public void save(SemanaLetiva o) {
        ...
        Calendario cal = p.getId().getCalendario();
        Integer campusId = cal.getId().getCampus().getId(),
                ano = cal.getId().getAno();
        // Verifica se existe ID
        if (o.getId().getId() == null) {
            o.getId().setId(nextVal(p.getId().getNumero(), ano, campusId));
        }
        if (o.isDeleted()) {
            this.delete(o);
        } else {
            ...
            String filter = String.format("@numero=%d and @pNumero=%d and @ano=%d and @campusId=%d",
                    o.getId().getId(), p.getId().getNumero(), ano, campusId);
            super.save(o, filter);
        }
    }
