...
public class MetodologiaDaoXML extends AbstractDaoXML<Metodologia> {
    ...
    public MetodologiaDaoXML(URL url) throws IOException, ParserConfigurationException, TransformerException {
        super("metodologia", url, "Metodologia", "metodologia", MetodologiaFactory.getInstance());
    }
    ...
    protected Metodologia createObject(Element e, Object ref) {
        try {
            ...
            Integer detalhamentoSequencia = Integer.parseInt(e.getAttribute("detalhamentoSequencia")),
                    planoDeEnsinoId = Integer.parseInt(e.getAttribute("planoDeEnsinoId")),
    ...
    public void save(Metodologia o) {
        ...
        Integer planoId = detalhamento.getPlanoDeEnsino().getId(),
                detalhamentoSeq = detalhamento.getId().getSequencia(),
                undId = detalhamento.getPlanoDeEnsino().getUnidadeCurricular().getId().getId(),
                cursoId = detalhamento.getPlanoDeEnsino().getUnidadeCurricular().getCurso().getId().getId(),
                campusId = detalhamento.getPlanoDeEnsino().getUnidadeCurricular().getCurso().getCampus().getId();
        if (o.getId().getSequencia() == null) {
            o.getId().setSequencia(this.nextVal(detalhamentoSeq, planoId, undId, cursoId, campusId));
        }
        ...
        if (o.isDeleted()) {
            ...
            String filter = String.format("@sequencia=%d and @detalhamentoSequencia=%d and @planoDeEnsinoId=%d and @unidadeCurricularId=%d and @cursoId=%d and @campusId=%d",
                    o.getId().getSequencia(), detalhamentoSeq, planoId, undId, cursoId, campusId);
            super.save(o, filter);
        }
    }
